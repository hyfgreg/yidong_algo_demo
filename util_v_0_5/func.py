...
import operator
import pymongo
from util_v_0_5.common_func import get_distance, get_area
...
from util_v_0_5.data import ydline, ydstation, stationTime, sbwstation,sbwtime,sbwline, busSchedule
...
def out_put_sbw_trip(otime,duration,price,sbwplans,o_wt=0,d_wt=0):
    ...
    pc.insert(3,price)
    o=sbwplans[0][0]
    # print(sbwplans[0][1],sbwplans[0][1])
    oline=sbwline[sbwplans[0][1]]
    odir=sbwplans[0][-2]
    d=sbwplans[-1][-1]
    dline=sbwline[sbwplans[-1][1]]
    ddir=sbwplans[-1][-2]
    if odir in sbwtime[o][oline]['first']:
        first=sbwtime[o][oline]['first'][odir]
        if ddir in sbwtime[d][dline]['last']:
