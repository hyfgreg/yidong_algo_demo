...
import pymongo
# import redis
from util_v_0_3.common_func import get_distance, get_area
...
from util_v_0_2.data import ydline, ydstation, stationTime, sbwstation, busSchedule
...
def plan_sbw_trip(sbw_os,sbw_ds,otime):
    ...
    if otime > starttime and otime < endtime:
        ...
        plantype,duration,price,plans=get_sbwplan(sbw_ostation, sbw_dstation)
        if plantype!=False:
            plan=out_put_sbw_trip(otime,duration,price,sbwplans,o_wt,d_wt)
...
def sort_trips_by_time(tripsdict):
    ...
    if len(all_trips)>0:
        ...
        if tripsdict['subwayPlan'][0]!=False:
            maxtime=trips['subwayPlan'][0][0]['totalDuration']+15
