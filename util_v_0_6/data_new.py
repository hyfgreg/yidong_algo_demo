...
import os
import copy
from util_v_0_6.yd_class import Station, Line
from util_v_0_6.common_func import get_area, get_distance
...
def data_init(day_of_data=None):
    ...
    mylogger.info('载入数据{}'.format(Config_dev.base_folder_yidong + '/routeListSet/routeListSet{}.json'.format(day_of_data)))

    # 读取站点列表的json文件
    with open(Config_dev.base_folder_yidong + '/routeStationList/routeStationList{}.json'.format(day_of_data), 'r',
              encoding='utf-8') as f:
        sdata = json.loads(f.read())
    mylogger.info('载入数据{}'.format(Config_dev.base_folder_yidong + '/routeStationList/routeStationList{}.json'.format(day_of_data)))

    # 读取各站时刻表的json文件
    with open(Config_dev.base_folder_yidong + '/routeStationTime/routeStationTime{}.json'.format(day_of_data), 'r',
              encoding='utf-8') as f:
        tdata = json.loads(f.read())
    mylogger.info('载入数据{}'.format(Config_dev.base_folder_yidong + '/routeStationTime/routeStationTime{}.json'.format(day_of_data)))

    # 读取排班表的json文件
    with open(Config_dev.base_folder_yidong + '/busSchedule/busSchedule{}.json'.format(day_of_data), 'r',
              encoding='utf-8') as f:
        bdata = json.loads(f.read())
    mylogger.info('载入数据{}'.format(Config_dev.base_folder_yidong + '/busSchedule/busSchedule{}.json'.format(day_of_data)))

    # 读取地铁站点列表的json文件
    sbw_sdata = []
    with open(Config_dev.base_folder + '/data/stations_time.dat', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            sbw_sdata.append(json.loads(line))

    ## 读取地铁线路列表的json文件
    sbw_ldata = []
    with open(Config_dev.base_folder + '/data/lines.dat', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            sbw_ldata.append(json.loads(line))
    ...
    ## 0529更新 加入地铁首末班数据/地铁线路数据
    # DICT SBWSTATION
    # 将获取到的地铁json站点数据整理成站点类的形式，存在key为地铁站点id，value为站点类的字典中
    # DICT SBWTIME
    # 将获取到的地铁json站点首末班次时间，存在key为地铁站点name，value为首末班车时间的字典中
    sbwstation = {}
    sbwtime = {}
    for i in range(len(sbw_sdata)):
        ...
        sbwtime[name] = sbw_sdata[i]['operating_time']
    with open('sbwtime.json', 'wb') as f:
        f.write(json.dumps(sbwtime, ensure_ascii=False, indent=4).encode('utf-8'))

    sbwline = {}
    for i in range(len(sbw_ldata)):
        id = sbw_ldata[i]['id']
        name = sbw_ldata[i]['name']
        sbwline[name] = [id, sbw_ldata[i]['stations']]

    # yd站点半径800米内地铁站点定义为周边站点【1】，获取周边站点集合并添加到对应站点类中
    nearby_sbwstops = {}
    for key in ydstation:
        nearby_sbwstops[key] = []
        [c_lat, c_lng] = ydstation[key].get_location()
        minlat, maxlat, minlng, maxlng = get_area(c_lat, c_lng, 0.8)
        n_stops = {}
            ...
            for k in tdata[key]:
                ...
                for c in tdata[key][k]:
                    ...
                    for i in range(len(tdata[key][k][c])):
                        ...
                        timeStr = tdata[key][k][c][i]['time'].strip().replace('：', ':').replace(';',':')
                        if timeStr != '':
    ...
    return ydline, ydstation, stationTime, sbwstation, sbwtime, sbwline, busSchedule


def set_data_to_rds():
    # ydline, ydstation, stationTime, sbwstation, sbwtime, sbwline, busSchedule = data_init()
    name = ['ydline', 'ydstation', 'stationTime', 'sbwstation', 'sbwtime', 'sbwline', 'busSchedule']
    for name,func in zip(name,data_init()):
        Config_dev.rds.set(name,func)
