...
import operator
import pymongo
from util_v_0_6.common_func import get_distance, get_area
...
# from util_v_0_6.data import ydline, ydstation, stationTime, sbwstation,sbwtime,sbwline, busSchedule
...
def out_put_sbw_trip(otime,duration,price,sbwplans,o_wt=0,d_wt=0):
    ...
    for key in sbwline:
        if key.find(olname)!=-1:
            isOin=False
            isO_Din=False
            for i in range(len(sbwline[key][1])):
                if o==sbwline[key][1][i]['station']:
                    isOin=True
                if o_d==sbwline[key][1][i]['station']:
                    isO_Din=True
    ...
    for key in sbwline:
        if key.find(dlname)!=-1:
            ...
            isD_Oin=False
            for i in range(len(sbwline[key][1])):
                if d==sbwline[key][1][i]['station']:
                    isDin=True
                if d_o==sbwline[key][1][i]['station']:
                    isD_Oin=True
            if isDin==True and isD_Oin==True:
                dline=sbwline[key][0] 
    ddir=sbwplans[-1][-2]
    if oline!="" and dline!="":
        if odir in sbwtime[o][oline]['first']and ddir in sbwtime[d][dline]['last']:
            first=sbwtime[o][oline]['first'][odir]
            last=sbwtime[d][dline]['last'][ddir]
...
def plan_yd_yd_trip(os,ds,otime,olatlng,dlatlng,membertype=0):
    ...
    if plantype!=-1:
        ...
        if plans!=None:
            return True,yd_yd_json_out_put(plans)
        else:
...
def plan_sbw_trip(sbw_os,sbw_ds,otime):
    ...
    if plantype!=False:
        ...
        joutput=sbw_sbw_json_out_put([plan])
        return True,joutput
    else:
        return False,'route limit'
...
def yd_yd_json_out_put(plans):
    ...
    for plan in plans:
        ...
        joutput[i]['part']=[]
        joutput[i]['part'].append({'partNo':1,'partType':'ydplan','partDetail':ydpart_json_out_put(plan)})
    return joutput

def sbw_sbw_json_out_put(plans):
    joutput=[]
    n=0
    for plan in plans:
        i=n
        ...
        joutput[i]['totalPrice']=plan[3]
        joutput[i]['part']=[]
        joutput[i]['part'].append({'partNo':1,'partType':'subwayplan','partDetail':sbwpart_json_out_put(plan)})
    return joutput
...
def yd_sbw_json_out_put(plans,type):
    ...
    for plan in plans:
        ...
        joutput[i]['part']=[]
        if type==0:
            joutput[i]['part'].append({'partNo':type+1,'partType':'ydplan','partDetail':ydpart_json_out_put(plan[type+4])})
            joutput[i]['part'].append({'partNo':2-type,'partType':'subwayplan','partDetail':sbwpart_json_out_put(plan[5-type])})
        else:
            joutput[i]['part'].append({'partNo':2-type,'partType':'subwayplan','partDetail':sbwpart_json_out_put(plan[5-type])})
            joutput[i]['part'].append({'partNo':type+1,'partType':'ydplan','partDetail':ydpart_json_out_put(plan[type+4])})
    return joutput
...
def plan_trip(o_lat,o_lng,d_lat,d_lng,o_time,membertype=0):
    ...
    os,ds=get_odstops(o_lat,o_lng,d_lat,d_lng,800)
    ...
    if ('subwayPlan' not in trips) or ('subwayPlan' in trips and trips['subwayPlan'][1]=='route limit'):
        sbw_os,sbw_ds=get_sbwstops(o_lat,o_lng,d_lat,d_lng,2000)
        if sbw_os[1]!=None and sbw_ds[1]!= None and sbw_os[1]!= sbw_ds[1]:
            result4,trip4=plan_sbw_trip(sbw_os,sbw_ds,otime)
            if result4==True:
                trips['subwayPlan']=[True,trip4]
